...
- The **WP diff range**: the cumulative diff from the WP's pre-WP baseline (the
  ...
  WP-diff range `goalforge-verify` already computed** for its semantic gate. Collect the
...
2. **Read the diff + neighbors.** Collect the files changed in the **WP diff
   range** (pre-WP baseline → HEAD — the range `goalforge-verify` computed, not per-task
...
4. **Report** at the configured depth (below). Then return control to
   `goalforge-verify` — the audit is advisory and the WP status is unaffected.
...
that loop-back is recorded in the feature's `recap.md` via `goalforge-recap`
...
`goalforge-verify` fast. The report shape:

```
### goalforge-watchdog gap audit — <wp-slug>
...
request from `goalforge-verify`/the user). Not the default — deep audits cost time.

## Wiring

`goalforge-watchdog` is **folded into `goalforge-verify`'s single semantic pass** (not a
separate per-task pass): `goalforge-verify` invokes it once, after its acceptance gate
...
any material gap as a `recap.md` loop-back via `goalforge-recap`).
