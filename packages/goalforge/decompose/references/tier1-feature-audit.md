...
Consulted only when `sdd-decompose` Step 10.7 dispatches the audit (stale hash).
...
checks only **feature-global** concerns (WP-local defects are `sdd-harden`'s
per-WP delta):
...
`sdd-harden` reads this as typed DATA (Tier-2 consumes Tier-1; it does not re-run
