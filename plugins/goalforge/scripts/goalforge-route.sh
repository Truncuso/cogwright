...
#   clear      — no signal (fast) or >=1 strong signal (full)
#   borderline — only weak signal(s) tripped, or ## Goal missing/empty
#                (unclassifiable => safe default full). Caller confirms with
#                the human on borderline; clear routes silently.
#
# Consumed by sdd-capture as typed DATA, never as instructions.
# Exit 0 on success (verdict in JSON). Exit 1 on errors (stderr).
set -uo pipefail

usage() { echo "usage: sdd-goal-route.sh <feature-overview.md> | --self-test" >&2; }

classify() {
    # arg: <overview.md path>
    SDD_ROUTE_NS="${SDD_ROUTE_NS:-4}" SDD_ROUTE_NQ="${SDD_ROUTE_NQ:-3}" \
    SDD_ROUTE_NF="${SDD_ROUTE_NF:-5}" python3 - "$1" <<'PY'
...
# Pinned route: echo back, no re-classification (idempotent).
mr = re.search(r"^route:\s*['\"]?(fast|full)", fm, re.MULTILINE)
if mr:
    print(json.dumps({"route": mr.group(1), "confidence": "pinned",
...
tripped = [s for s, hit in [("R1", r1), ("R2", r2), ("R3", r3), ("R4", r4), ("R5", r5)] if hit]
strong = [s for s in tripped if s != "R5"]
goal_missing = not goal_lines

if strong:
    route, confidence = "full", "clear"
elif goal_missing or tripped:          # unclassifiable, or weak-only
    route, confidence = "full", "borderline"
else:
...
self_test() {
    ...
    echo "=== sdd-goal-route.sh --self-test ==="
    d="$(mktemp -d)"
    trap 'rm -rf "$d"' RETURN
...
EOF
    ...
    if [[ "$(field "$out" .route)" == "fast" && "$(field "$out" .confidence)" == "clear" ]]; then
        ok "F1 small goal -> fast/clear"
    else no "F1 small goal -> $out"; fi

    # F2: migration task_type -> full/clear via R1
    printf -- '---\ntask_type: migration\n---\n## Goal\n\nmove files\n' > "$d/mig.md"
    out="$(classify "$d/mig.md")"
    if [[ "$(field "$out" .route)" == "full" && "$(field "$out" .tripped[0])" == "R1" ]]; then
        ok "F2 migration -> full/R1"
    else no "F2 migration -> $out"; fi

    # F3: many open questions -> full/clear via R3
    printf -- '## Goal\n\nbig thing\n\n## Open Questions\n\n- a?\n- b?\n- c?\n' > "$d/oq.md"
    out="$(classify "$d/oq.md")"
    if [[ "$(field "$out" .route)" == "full" && "$(field "$out" .tripped[0])" == "R3" ]]; then
        ok "F3 3 OQs -> full/R3"
    else no "F3 3 OQs -> $out"; fi

    # F4: keyword only -> full/borderline (weak signal => confirm)
    printf -- '## Problem\n\ntouches auth flow\n\n## Goal\n\nsafer login copy\n' > "$d/kw.md"
    out="$(classify "$d/kw.md")"
    if [[ "$(field "$out" .route)" == "full" && "$(field "$out" .confidence)" == "borderline" ]]; then
        ok "F4 keyword-only -> full/borderline"
    else no "F4 keyword-only -> $out"; fi

    # F5: missing goal -> full/borderline (unclassifiable)
    printf -- '## Problem\n\nsomething\n' > "$d/nogoal.md"
    out="$(classify "$d/nogoal.md")"
    if [[ "$(field "$out" .route)" == "full" && "$(field "$out" .confidence)" == "borderline" ]]; then
        ok "F5 no goal -> full/borderline"
    ...
    if [[ "$(field "$out" .route)" == "fast" && "$(field "$out" .confidence)" == "pinned" ]]; then
        ok "F6 pinned route -> echoed"
    else no "F6 pinned route -> $out"; fi

    # F7: >=5 path tokens -> full via R4
    printf -- '## Goal\n\ntouch a/b.md c/d.md e/f.md g/h.md i/j.md\n' > "$d/paths.md"
    out="$(classify "$d/paths.md")"
    if [[ "$(field "$out" .route)" == "full" && "$(field "$out" .tripped[0])" == "R4" ]]; then
        ok "F7 5 paths -> full/R4"
