...
`chain.yaml` (sibling file) declares the sequence. `sdd-run` does not
...
adapts to the goal's size: `sdd-run` reads the feature overview's `route:`
frontmatter (stamped by `sdd-capture` via `goalforge-route.sh` — capture is the
...
- **Escalation:** ANY fast-gate trip leaves the WP at `spec` and re-enters the
  ...
  are the safety net.
- **Never skipped:** `sdd-verify` — every route ends at the single semantic
  gate; the outcome→verification contract is route-independent.

Full fast-path runbook: `sdd-capture` §Fast path.
...
When `SDD_AUTONOMY=unattended` is set (the `autopilot` driver sets it), `sdd-run`
...
(`interactive`, the default), gate behaviour is unchanged. Note: `sdd-run` must
...
|--------------|---------------|----------------|-------------------------------|
| `/spec`      | `capture`     | `sdd-capture`  | Starts chain; elicits raw input, writes `overview.md` |
| `/plan`      | `decompose`   | `sdd-decompose`| Expects feature `status: ready`; breaks spec into WPs  |
| `/implement` | `execute`     | `sdd-execute`  | Expects WP `status: ready`; runs execute sub-cycle    |
| `/verify`    | `verify`      | `sdd-verify`   | Expects WP `status: executing`; runs verification     |

`sdd-run` resolves which step to enter by checking the target WP's (or
feature's) `status:` frontmatter field before dispatching.

## Resume mid-chain

When invoked without an explicit entry command, `sdd-run` reads the current
WP's `status:` frontmatter and resumes at the next actionable step:

| Current `status:` | Next step  | Child skill    | Gate           |
|-------------------|------------|----------------|----------------|
| `draft`           | `spec`     | `sdd-spec`     | human-gated    |
| `ready` (feature) | `decompose`| `sdd-decompose`| automatic      |
| `hardened`        | `harden`→`ready` | `sdd-harden` | human-gated  |
| `ready` (WP)      | `execute`  | `sdd-execute`  | automatic      |
| `executing`       | `verify`   | `sdd-verify`   | automatic      |
...
auto`; sdd-harden Step 2.3 / state-machine.md §Policy). Outside that class,
`sdd-run` calls `AskUserQuestion` at each gated boundary. On the fast route
...
`skill-router sdd-run --dry-run` (or passing `--dry-run` when invoking
`sdd-run` directly):
...
`draft → ready` (feature, via sdd-spec) and `hardened → ready` (WP, via sdd-harden) are human-gated
