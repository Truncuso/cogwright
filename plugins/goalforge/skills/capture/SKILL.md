---
name: sdd-capture
description: "Capture free-text user intent and scaffold the initial feature plan files. Creates plans/<feature>/overview.md (status: draft) by stamping the feature-overview template, then classifies the chain route (fast vs full) via sdd-goal-route.sh and stamps `route:` — a small clean goal takes the fast path (single WP, no spec.md); everything else takes the full chain. The feature-level todo.md is auto-generated later by sdd-decompose via sdd-rollup.sh after WPs are created. Idempotent: if the folder already exists, updates in place without clobbering. Use when starting a new feature from scratch, after brainstorming, or when invoked via /spec. Trigger: the user describes a new feature, change, or work item in natural language."
metadata:
  version: 1.1.0
hooks:
  Stop:
    - hooks:
        - type: command
          command: "$HOME/.claude/scripts/skill-measure.sh sdd-capture"
        - type: command
          command: "$HOME/.claude/hooks/skill-trace.sh sdd-capture:stop"
---
...
Note: `plans/<feature>/todo.md` is NOT stamped here — `sdd-decompose` auto-generates
it via `sdd-rollup.sh` after WPs are created (`feature-todo.md` documents its shape).
...
(empty, filled by `sdd-decompose`), **Links** (keep `spec.md`, `todo.md`).
...
`sdd-frontmatter-touch.sh` hook also bumps these on every Write/Edit; the skill
...
```bash
bash ~/.claude/skills/sdd/scripts/sdd-goal-route.sh <PLANS_ROOT>/<feature>/overview.md
...
Next (route: full): run sdd-spec to produce the design document.
Next (route: fast): fast-path continuation below — single WP, no spec.md.
(Feature todo.md will be generated by sdd-decompose via sdd-rollup.sh after decomposition.)
```
...
1. **Author the single WP** via the one-WP authoring mode (never hand-author):
   ```bash
   # sdd-decompose --add-wp authors wp-01-<slug>/ with a complete goal block
   # (outcome + verification + constraints/boundaries), todo.md, task files,
   # rollup, and the feature WP table. See sdd-decompose §Add-WP mode.
   ```
   ...
   — **re-route to `full`** (flip `route:` with a note) and hand off to `sdd-spec`.
2. **Gate deterministically** (all three, typed DATA):
   - `sdd-validate.sh` — WP goal-block integrity (fatal on malformed);
   - `sdd-open-questions-gate.sh --check <wp>/overview.md` — must print `0`;
   - `sdd-wp-complexity.sh <wp>` — verdict `simple`, AND `severity ≤ MEDIUM`, AND
...
3. **Record the goal hash** — BEFORE the `--mode auto` advance below. The fast WP
   is born `goal_approved_version: null` (`sdd-decompose --add-wp`), and wp-01's
   `→ready` gate refuses any WP whose goal hash is unrecorded. The fast path's
   `--mode auto` door records the hash first — exactly as `sdd-harden`'s own
...
5. **Escalate on any trip.** A failed gate, `complex` verdict, HIGH/CRITICAL
   ...
   `spec` and run full `sdd-harden` (Tier-2 review + interview + human gate).
   Never force a tripped WP through.
6. **Execute + verify unchanged:** `sdd-execute` then `sdd-verify` — every path
   ends at the single semantic gate; fast never skips verification.

## Constraints

- **Never** advance `status:` beyond `draft`.
- **Never** create WP folders or `spec.md` — those belong to `sdd-spec`/`sdd-decompose`
  (the fast path *delegates* WP authoring to `sdd-decompose --add-wp`; capture writes only `overview.md`).
...
- Idempotency on an existing overview preserves `status:` as-is — a prior `sdd-spec` advance to `ready` is NOT reset to `draft` on re-capture. Check current status before presenting next-step guidance.
