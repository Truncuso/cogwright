...
1. **Before the move — capture references.** For each path the WP relocates, run the
   ...
   ```bash
   bash "$COGWRIGHT_ROOT"/plugins/goalforge/scripts/goalforge-rewire-impact.sh <old-path>
...
2. **After the move — dangling-reference gate.** Once the file is moved and the
   ...
   ```bash
   bash "$COGWRIGHT_ROOT"/plugins/goalforge/scripts/goalforge-rewire-impact.sh --post-move <old-path>
