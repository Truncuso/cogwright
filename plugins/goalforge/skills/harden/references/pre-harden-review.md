...
| 7 | **Unread prescribed prose** | a task step dictates literal text to insert into a file and the task `verify:` only greps a substring of it, so the prescribed sentence is never read as the artifact it will become — it conflates two paths (write target vs template/pointer), or carries reviewer meta-annotation (`— exact absolute string`) that will ship verbatim | read every prescribed insert text as the FINAL shipped sentence: each path named by its role, no meta-annotation, no invented content beyond the stated constraint — and name what the substring grep cannot assert |
