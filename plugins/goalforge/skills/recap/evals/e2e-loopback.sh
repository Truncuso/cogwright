...
#
# This proves sdd-recap (the living trace) and sdd-watchdog (the gap audit whose
# finding triggers a loop-back) integrate into the sdd-execute/sdd-verify loop
...
# Make a fixture commit and echo its short hash (models an sdd-execute task commit).
fixture_commit() {
  local msg="$1"
...
echo "=== sdd-recap e2e: execute -> verify -> gap -> loop-back -> re-execute -> verify ==="

# 1. sdd-execute Step 0: recap init for the feature
bash "$RECAP_SH" init "$RECAP" "fixture-feature"

# 2. sdd-execute: task-01 commit -> Step 8.5 recap append-task
C1="$(fixture_commit 'feat(task-01): first task')"
( cd "$TMPDIR" && bash "$RECAP_SH" append-task "$RECAP" "$WP" "task-01" "ok" )

# 3. sdd-execute: task-02 commit -> Step 8.5 recap append-task
...
# 4. sdd-verify: acceptance passes, then sdd-watchdog audits the diff and finds a
#    semantic gap (missing test at the changed seam). The gap finding triggers a
#    loop-back -> sdd-execute Step 8.5 records it. (Watchdog is model-driven; the
...
# 5. sdd-execute re-executes task-02 with the fix -> new commit -> append-task
...
# 6. sdd-verify: record the WP as ONE row carrying the SINGLE WP commit at WP
