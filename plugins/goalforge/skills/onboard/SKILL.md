...
description: >
  ...
  sdd-install-hooks.sh), and stamps a minimal ## SDD pointer into the repo
...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "$HOME/.claude/scripts/skill-measure.sh sdd-onboard"
---
...
2. **Install the pre-commit hook** — delegates entirely to
   `sdd-install-hooks.sh`, which handles the chain-safe append-to-existing-hook
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/sdd-onboard.sh [<repo-dir>]
```
...
```
$HOME/.claude/skills/sdd/scripts/sdd-install-hooks.sh <repo-dir>
```

`sdd-onboard` does not reimplement hook logic.
...
- **Current vocabulary only.** The stamped `## SDD` block uses the current
  ...
  `sdd-verify`). It does NOT reference the legacy GSD 10-state lifecycle. Keep the
...
- **Not a GitHub-issues setup.** This skill does not create GitHub issues,
  ...
  `setup-sdd-repo` surface. `sdd-onboard` is purely on-disk scaffolding.
...
- **The hook validates TOUCHED features under `--strict`, not commit hash.**
  ...
  happens in `sdd-verify` at verify-time, not in the pre-commit hook.
