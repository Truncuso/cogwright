...
2. **Install the pre-commit hook** — delegates entirely to
   ...
   case and the idempotency marker (`# >>> goalforge-pre-commit >>>`).
...
```
"$COGWRIGHT_ROOT"/plugins/goalforge/scripts/goalforge-install-hooks.sh <repo-dir>
```
...
- **Not a GitHub-issues setup.** This skill does not create GitHub issues,
  ...
  `setup-goalforge-repo` surface. `goalforge-onboard` is purely on-disk scaffolding.
- **Idempotent everywhere.** Re-running is safe: `plans/` is not clobbered,
  ...
  (`# >>> goalforge-pre-commit >>>`) prevents double-installation.
