...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "$HOME/.claude/scripts/skill-measure.sh sdd-archive"
        - type: command
          command: "$HOME/.claude/hooks/skill-trace.sh sdd-archive:stop"
---
...
`sdd-verify` advances a feature to `completed` (last-WP rule) and then *offers*
...
```bash
bash ~/.claude/skills/sdd/scripts/sdd-archive.sh <feature> --relocate \
...
A feature must reach `completed` (all sibling WPs `verified`, via `sdd-verify`'s
...
transition in `sdd-verify` (role `integration-review`). This step is the
...
- **Confirm the sdd-verify integration review ran and its findings are resolved**
...
## Steps 2–4 — Mechanical core (delegated to `sdd-archive.sh`)

The frontmatter edit, physical move, and validator gate are performed
**deterministically by a script** — `~/.claude/skills/sdd/scripts/sdd-archive.sh`
— so the same logic is drivable unattended (the `sdd-archive-batch.sh` loop). The
...
```bash
bash ~/.claude/skills/sdd/scripts/sdd-archive.sh <feature> \
...
- **Behavior:** WARN by default (prints the offending refs + the relocate-then-move
  remedy, exit 0 — non-blocking so the unattended `sdd-archive-batch.sh` loop is
...
longer exist):
```bash
bash ~/.claude/skills/sdd/scripts/sdd-ensure-committed.sh <PLANS_ROOT>/_archived/<feature>
# and, under --supersedes:
bash ~/.claude/skills/sdd/scripts/sdd-ensure-committed.sh <PLANS_ROOT>/_archived/<old>
```
...
`sdd-archive` is **NOT** wired into `chain.yaml`. Archival is a terminal,
...
```
sdd-archive REFUSED — precondition not satisfied:

Feature: <feature>
status: <offending-status>   (required: completed)

A feature must be `completed` (all WPs verified via sdd-verify) before it can be
...
```
sdd-archive REFUSED — supersedes target not found:
...
- The status gate is fail-closed on exactly `completed` — `active`, `ready`, `draft`, and an already-`archived` feature all REFUSE in the default mode. There is no "force archive"; advance the feature to `completed` via `sdd-verify`'s last-WP rule first. The one exception is `--relocate`, whose gate is the *inverse* (`status: archived` required) and which only moves a stranded archived feature into `_archived/` — it never flips a non-archived status.
...
- After the physical move, `sdd-ensure-committed.sh` must be called on `<PLANS_ROOT>/_archived/<feature>`, NOT on the original path (which no longer exists after the move).
- `sdd-validate.sh --strict <PLANS_ROOT>` must walk `_archived/` to resolve cross-feature `supersedes/superseded_by` edges — pass `<PLANS_ROOT>` (not `<PLANS_ROOT>/<feature>`) so the validator sees both sides, including features that have already been moved into `_archived/`.
