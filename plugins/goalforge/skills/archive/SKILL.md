...
- Feature terminal status is `archived` (reached from `completed`, which is the mandatory pre-archive feature status — see the gate above); the WP terminal status is `verified`, with `archived` as a second WP terminal set only by explicit out-of-band edit, never by a transition edge and never by any goalforge script. Never write `status: verified` to a feature overview, and never `status: completed` to a WP — the enums do not overlap at the terminal end, with `archived` as the one deliberate exception (legal on both). A WP at `archived` is **dep-satisfying**: `goalforge-frontier.sh` and `goalforge-validate.sh` treat it exactly as `verified` when resolving a sibling's `depends_on`.
