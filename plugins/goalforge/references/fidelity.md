...
| `harden` | Step 1 applies the trigger to flag a spike candidate and routes it to the `prototype` skill (handoff mode `prototype`); the `interview` plugin engine stays the sole resolver for everything else. Escalation refinement is owned by `goalforge-interview` (see `interview/`). — spike spec: `${CLAUDE_PLUGIN_ROOT}/skills/prototype/references/spike-spec.md` |
