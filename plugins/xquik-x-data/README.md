...
2. Connect the bundled remote MCP server through its OAuth flow.
3. For direct REST API work, keep `XQUIK_API_KEY` in the runtime environment or an approved secret store.
4. Use the `xquik-x-data` skill when a task needs X post search, user lookups, media retrieval, monitors, webhooks, or SDK references.
...
The remote MCP server uses OAuth. For direct REST requests, keep
`XQUIK_API_KEY` in your local environment or approved secret store. Do not
paste API keys into prompts, issues, logs, or repository files.
