...
4. If MCP is useful, configure the server from `.mcp.json` and authenticate through its OAuth flow.
5. For direct REST calls, keep `XQUIK_API_KEY` in an approved runtime secret path.
6. Keep keys out of prompts, logs, commits, issues, and PRs.
7. Prefer read-only discovery unless the user explicitly asks for an action that changes account state.
8. Link users to `https://docs.xquik.com` and the source skill when they need setup details.
